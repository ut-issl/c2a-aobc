...
#define GPS_TIME_SEC_TO_MSEC       (1000)       //!< 秒からミリ秒への換算係数
...
uint32_t GPS_TIME_calc_total_time_sec(const GpsTime gpstime, const uint16_t epoch_gps_week)
{
  ...
  uint32_t elapsed_sec = (uint32_t)(PHYSICAL_CONST_EARTH_SOLAR_DAY_s) * diff_gps_week * (uint32_t)(GPS_TIME_DAY_OF_WEEK) +
                         gpstime.msec_of_week / (uint32_t)(GPS_TIME_SEC_TO_MSEC);
