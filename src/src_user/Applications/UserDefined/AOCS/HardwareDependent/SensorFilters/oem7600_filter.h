...
typedef struct
{
  ...
  SpikeFilter_Config gps_time_total_sec_spike_filter_config;                  //!< スパイク除去フィルタ設定パラメータ for 受信機時刻通算秒
  ...
  C2A_MATH_ERROR gps_time_total_sec_filter_error;                    //!< スパイク除去フィルタ入力判定ログ for 受信機時刻通算秒

  uint32_t gps_time_total_sec_epoch_week;                            //!< 受信機時刻通算秒の元紀とするGPS週番号
